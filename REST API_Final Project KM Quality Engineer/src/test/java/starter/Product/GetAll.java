...
public class GetAll {
    ...
    @Step("I set method GET at api endpoints for get all data product")
    public String setMethodGet() {
        return this.endpoint + "/products";
    }

    @Step("I send method GET HTTP request for get all data product")
    public void sendMethodGet() {
        SerenityRest.given().get(this.setMethodGet());
    }

    @Step("I receive valid HTTP response code 200 for get all data product")
    ...
    public void receiveValidMessage() {
        restAssuredThat(response -> response.body("data[0].ID", equalTo(19   )));
    }
}
